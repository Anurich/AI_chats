...
async def summarization_doc(requestQuery: QueryRequest):
    ...
    chat_tool = Chatwithdocument(vector_db=vector_doc.vector_db if len(vector_doc.page_texts) > 0 else "" ,llm=llm, user_id=requestQuery.user_id, chat_id=requestQuery.chat_id)
...
async def file_search(requestQuery: QueryRequest):
    file_search_by_keyword = Filesearchbykeyworddescrp(llm=llm, client=client, persist_directory="search_by_keyword",user_id=requestQuery.user_id, chat_id=requestQuery.chat_id)
