...
class Chatwithdocument(CustomLogger):
    ...
    def run_chat(self, query: str):
        ...
        output = # The `response` variable in the `run_chat` method is storing the output generated by
        # the chatbot in response to a user query. It contains the processed answer along
        # with additional information such as sentiment analysis results and metadata related
        # to the document that was used to generate the response. The `response` variable is
        # then used to append the query and output to the chat history for future reference.
        response["answer"]
